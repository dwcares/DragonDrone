...
class AnalyzeImage: NSObject {
    ...
    final let apiURL = "/vision/v1.0/analyze"
    ...
    final func analyzeImageWithRequestObject(_ requestObject: AnalyzeImageRequestObject, completion: @escaping (_ response: AnalyzeImageObject?) -> Void) throws {
        ...
        if CognitiveServices.ComputerVisionKey.characters.count <= 0 {
        ...
        let requestURL = URL(string: CognitiveServices.ServiceURL + apiURL + "?" + parameters)!
        ...
        request.setValue(CognitiveServices.ComputerVisionKey, forHTTPHeaderField: "Ocp-Apim-Subscription-Key")
    ...
    private func objectFromDict(_ dict: [String : AnyObject]?, data: Data) -> AnalyzeImageObject {
        ...
        if let faces = dict?["faces"] as? [[String : AnyObject]] {
            ...
            if faces.count >= 1 {
                analyzeObject.getEmotions({ emotionFaces in
                    ...
                    self.delegate?.finishedGeneratingObject(analyzeObject)
                })
            }
        }
        ...
        if containsFaces == false {
            delegate?.finishedGeneratingObject(analyzeObject)
        }
...
extension AnalyzeImage.AnalyzeImageObject {
    ...
    func getEmotions(_ completion: @escaping (_ response: [[String : AnyObject]]?) -> Void) {
        ...
        let path = CognitiveServices.ServiceURL + "/emotion/v1.0/recognize"
        print(path)
        ...
        let key = CognitiveServices.EmotionKey
        
        if key == "Emotion Key" {
            assertionFailure("Enter your Emotion API key in Info.plist")
        }
